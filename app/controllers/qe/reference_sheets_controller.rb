module Qe
  class ReferenceSheetsController < AnswerSheetsController
    ...
    def edit
      @answer_sheet = @reference_sheet = QeReferenceSheet.find_by_id_and_access_key(params[:id], params[:a])
      unless @answer_sheet
      ...
      if @answer_sheet.question_sheets.empty?
        @answer_sheet.question_sheets << QeQuestionSheet.find(@answer_sheet.question.related_question_sheet)
      end
    ...
    def get_answer_sheet
      @answer_sheet ||= QeReferenceSheet.find(params[:id])
